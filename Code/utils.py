...
def mode(data):
    ...
    [(item, contor)] = collections.Counter(data).most_common(1)
    return item
...
def histogram(values, _mode=0, bin_function=None):
...
def matrix_multiplication(X_M, *Y_M):
    ...
    def _mat_mult(X_M, Y_M):
        ...
        rezultat = [[0 for _ in range(len(Y_M[0]))] for _ in range(len(X_M))]
        for i in range(len(X_M)):
            for j in range(len(Y_M[0])):
                for k in range(len(Y_M)):
                    rezultat[i][j] += X_M[i][k] * Y_M[k][j]
        return rezultat
...
def turn_heading(heading, inc, headings=None):
    if headings is None:
...
class injection:
    ...
    def __exit__(self, tip, value, traceback):
...
def open_data(nume, mode='r'):
    aima_root = os.path.dirname(__file__)
    aima_file = os.path.join(aima_root, *['aima-data', nume])
...
def symbols(names):
    ...
    return tuple(Symbol(nume) for nume in names.replace(',', ' ').split())
